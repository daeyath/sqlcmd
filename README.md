...
Sqlcmd has an input prompt for entering data to parameters. Parameters are written into SQL scripts. For example, the '${Test}' parameter will display Test prompt when the script is run, and the user can enter the value. And parameter changed in SQL script with value is given.
